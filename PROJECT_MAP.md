...
**Две копии = две правды.** Домен и GitHub Pages сейчас живут независимо: пуш
в репозиторий обновляет только второй адрес. До объединения источников любая
выкатка делается в два места, иначе на домене остаётся старая версия.

Узел CDN в заголовке ответа домена — `yccdn-defra01fr5-*` (читается как
Франкфурт). Это противоречит правилу «только российский контур» и требует
проверки в консоли Yandex Cloud.
