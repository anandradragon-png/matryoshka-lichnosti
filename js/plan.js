...
/* Возможность → минимальный тариф, который её открывает.
   ...
   ПОЧЕМУ ЗДЕСЬ БОЛЬШЕ НЕТ РАЗДЕЛОВ ОТЧЁТА. Раньше было семь строк вида
   'report.dar', 'report.trends': тариф решал, какие разделы человек увидит,
   и бесплатный отчёт получался огрызком с припиской «остальное не отдали».
   Теперь разделы одинаковые у всех, а тариф решает глубину разбора — это
   `js/report/depth.js`. Здесь остаётся только то, что действительно «есть
   или нет»: за какой период вообще можно собрать отчёт. */
